...
const CreatePlaceForm = () => {
  ...
  const [allCategory, setAllCategory] = useState<Array<CategoryStateProps>>([])
  ...
  async function handleCategory(event: ChangeEvent<HTMLSelectElement>) {
  ...
  return (
    <div className=''>
      <div className='mt-4'>
        <h1 className='font-jost-300 text-2xl mb-6'>Create Place</h1>
      </div>
      ...
      <div className='fields w-full font-jost-400'>
        ...
        }} className='w-full focus:outline-none bg-trasparent' type="text" placeholder='Enter Place Name' /></div>
        <div className='w-full border text-xs border-gray-300 h-4 mt-3 rounded-sm flex items-center py-4 px-3'><input onChange={(e) => setentryState({ ...entryState, address: e.target.value })} className='w-full focus:outline-none bg-trasparent' type="text" placeholder='Enter Address' /></div>
        <div className='w-full border text-xs border-gray-300 h-4 mt-3 rounded-sm flex items-center py-4 px-3'><input onChange={(e) => setentryState({ ...entryState, slug: e.target.value })} className='w-full focus:outline-none bg-trasparent' type="text" placeholder='Enter Slug' /></div>

        <select id="category" onChange={handleCategory} className="bg-gray-100 focus:outline-none mt-4 border border-gray-200 text-gray-900 text-sm rounded-sm focus:ring-blue-500 focus:border-blue-500 block w-full p-2">
          {
            allCategory?.map((p,id) =>(
              <option key={id} value={p._id}>{p.name}</option>
            ))
          }
        </select>
