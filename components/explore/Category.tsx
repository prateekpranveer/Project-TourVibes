...
const Category = () => {
    ...
    if (isLoading) {
        ...
        return (
            ...
            <div className='mt-8 flex flex-wrap gap-8 cursor-pointer'>
                {
                    places?.map((p, id) => (
                            <div key={id} className='max-w-fit relative bg-gray-200 rounded-full shadow-xl text-black'>
                                <div className='absolute flex justify-center bg-black py-2 hover:bg-gray-700 rounded-full -top-2 shadow-lg cursor-pointer text-white font-jost-300 text-xs px-4'>
                                    <span className='tracking-widest'>Explore Tours</span>
                                </div>
                                <div className='absolute bg-green-600 hover:bg-green-700 text-white p-3 rounded-full cursor-pointer -bottom-2 -right-2'><ChevronRight/></div>
                                <div className='py-8 px-12 font-jost-300'>
